...
```
#### 5.什么叫做覆盖索引？
[面试三轮我倒在了一道sql题上——sql性能优化](https://www.cnblogs.com/mingyueyy/p/13046712.html)
```markdown
当sql语句的所求查询字段select列和查询条件字段where子句全都包含在一个索引中联合索引，可以直接使用索引查询而不需要回表。
这就是覆盖索引，通过使用覆盖索引，可以减少搜索树的次数，这就是 覆盖索引
只需要在一棵辅助索引树上就可以获取SQL所需要的所有列数据，不需要回表。
主键索引：主键索引的叶子节点保存着主键即对应行的全部数据。在InnoDB里，主键索引也被称为聚簇索引（clustered index）。
二级索引（非主键索引）：二级索引树中的叶子结点保存着索引值和主键值，当使用二级索引进行查询时，需要进行回表操作。
