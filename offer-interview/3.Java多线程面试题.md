...
```markdown
AQS:全称AbstractQueueSynchronizer，抽象队列同步器，
    这个类在java.util.concurrent.locks包下是一个底层同步工具类，用来构建锁或者其他同步组件的基础框架。
    ...
    **AQS就是基于volatile+CAS实现的锁模板；如果需要线程阻塞等待，唤醒机制，则使用LockSupport挂起、唤醒线程。**
