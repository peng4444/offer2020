...
```markdown
因为hashCode相同，不一定就是相等的（equals方法比较），所以两个对象所在数组的下标相同，"碰撞"就此发生。
又因为HashMap使用链表存储对象，这个Node会存储到链表中。
```
#### 4.你知道hash的实现吗？为什么要这样实现？
```markdown
JDK1.8中，是通过hashCode()的高16位异或低16位实现的：(h=k.hashCode())^(h>>>16)，
主要是从速度，功效和质量来考虑的，减少系统的开销，
...
[HashMap：从源码分析到面试题](https://www.cnblogs.com/CryFace/p/13436598.html)