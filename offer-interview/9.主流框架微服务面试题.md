...
AOP主要用来解决：在不改变原有业务逻辑的情况下，增强横切逻辑代码，根本上解耦合，避免横切逻辑代码重复
    ...
    @Aspect 对横切关注点逻辑进行模块化封装的 AOP 概念实体，包含多个 Pointcut 和相关 Advice 的定义。
...
```
### 19.spring中拦截器与过滤器的区别【5+】
```markdown
filter定义在web.xml中，拦截器定义在Spring.xml中；
filter需要实现filter接口，拦截器需要实现HandleInterceptor接口或者继承抽象类HandlerInterceptorAdapter；
