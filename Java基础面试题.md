...
### 1.进程和线程的区别与联系
```markdown
进程是⼀个独立的运行环境，各个进程之间互不干扰，而线程是在进程中执行的⼀个任务。每个进程资源相互独立。
一个进程在其执行过程中可以产生多个线程。多个线程共享进程的堆和方法区资源，但每个线程有自己的程序计数器、虚拟机栈和本地方法栈、
...
### 多线程start()和run()方法的区别，sleep()和yield()方法的区别
```markdown
