...
public TreeNode constructFromPrePost(int[] pre, int[] post) {
    ...
    public TreeNode help(int[] pre,int[] post,int prestart,int preend,int poststart,int postend){
        ...
        root.left = help(pre,post,prestart+1,prestart+1+index-poststart,poststart,index);
        root.right = help(pre,post,prestart+2+index-poststart,preend,index+1,preend-1);
        return root;
    }
```
#### 1028. 从先序遍历还原二叉树
...
public TreeNode recoverFromPreorder(String S) {
        ...
        for(int i = 0;i<len;i++){
            ...
            if(cntD!=0){
                TreeNode parent = map.get(cntD-1);
                if(parent.left==null) parent.left = tree;
...
```
### 二叉搜索树95,96,98,99,230,235,450,501,701,938,1008
#### 98. 验证二叉搜索树
```markdown
...
public int findSecondMinimumValue(TreeNode root) {
    ...
    public int dfs(TreeNode parent,TreeNode root){
        if(root==null) return -1;
