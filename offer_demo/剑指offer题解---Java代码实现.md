# 一、前言
本文主要记录个人刷剑指offer的过程，一来加深记忆，二来便于复习加强，题目部分为自己解答，
其他的有些参考借鉴了牛客还有其他博客上的答案，望各位大佬轻喷，侵删。
# 二、题目
#### 1、二维数组中的查找
题目描述：
...
解题思路：
```java 
//暴力求解 在两段范围内都是非降序，当不符合这个规律时，就找到了最小数字
...
public class Solution {
    ...
    public void help(char[] strs, int len){
        if(len==strs.length){
...
```
>>利用栈“先进后出”的思想来进行反转。将原始字符串通过空格字符" "分解为多个子字符串，保存为字符串数组。遍历该数组将子字符进栈，
  进栈后还需要添加" "。值得注意的是，最后进栈的子字符串之后无需添加" "，因为其逆序后为字符串首字符。
```javascript
import java.util.Stack;
public class Solution {
    public String ReverseSentence(String str) {
