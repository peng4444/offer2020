...
快捷创建线程池很简单，只需要调用Executors中相应的静态工厂方法即可
    Executors.newFixedThreadPoll(int);执行一个长期的任务，性能好很多。
    Executors.newSingleThreadExecutor();一个任务一个线程执行的任务场景。
    Executors.newCacheThreadPool();适用：执行短期异步的小程序或者负载轻的服务器。
    Java8 Executors.newWorkStealingPool(int);java8新增，使用目前机器上可以的处理器作为他的并行级别。
使用ThreadPoolExecutor创建线程池
生产上JDK提供的创建线程池的方法一个都不用，一般自定义线程池ThreadPoolExecutor使用。
```
### 5.线程池处理原理和流程
...
public ThreadPoolExecutor(int corePoolSize,
        ...
        　　5.workQueue:任务队列(阻塞队列)，当空闲线程不够，也不能再新建线程时，新提交的任务就会被放到任务队列种。
